import org.junit.jupiter.api.Assertions.*
import org.junit.jupiter.api.Test
import ru.sbpstu.jblab.slicebleListOf
import ru.sbpstu.jblab.*

internal class SlicesTest {

    @Test
    fun getFor1Dim() {
        val testList = slicebleListOf(0, 1, 2, 3, 4, 5, 6, 7, 8)
        assertEquals(testList[0..6], slicebleListOf(0, 1, 2, 3, 4, 5, 6))
        assertEquals(testList[6 downTo 0], slicebleListOf(6, 5, 4, 3, 2, 1, 0))
        assertEquals(testList[1..8 step 2], slicebleListOf(1, 3, 5, 7))
        assertEquals(testList[7 downTo 0 step 2], slicebleListOf(7, 5, 3, 1))
    }

    @Test
    fun getFor2Dim() {
        val testTable = slicebleListOf(
            slicebleListOf(1, 2, 3, 4),
            slicebleListOf(5, 6, 7, 8),
            slicebleListOf(9, 10, 11, 12),
            slicebleListOf(13, 14, 15, 16)
        )

        assertEquals(testTable[0..2, 0..2],
            slicebleListOf(
                slicebleListOf(1, 2, 3),
                slicebleListOf(5, 6, 7),
                slicebleListOf(9, 10, 11)
            ))

        assertEquals(testTable[1..3, 0..2],
            slicebleListOf(
            slicebleListOf(5, 6, 7),
            slicebleListOf(9, 10, 11),
            slicebleListOf(13, 14, 15)
        ))

        assertEquals(testTable[0..3, 0..1],
            slicebleListOf(
            slicebleListOf(1, 2),
            slicebleListOf(5, 6),
            slicebleListOf(9, 10),
            slicebleListOf(13, 14)
        ))

        assertEquals(testTable[0..3 step 2, 0..2],
            slicebleListOf(
            slicebleListOf(1, 2, 3),
            slicebleListOf(9, 10, 11)
        ))

        assertEquals(testTable[3 downTo 0, 0..1],
            slicebleListOf(
            slicebleListOf(13, 14),
            slicebleListOf(9, 10),
            slicebleListOf(5, 6),
            slicebleListOf(1, 2)
        ))

        assertEquals(testTable[3 downTo 0 step 2, 1 downTo 0],
            slicebleListOf(
            slicebleListOf(14, 13),
            slicebleListOf(6, 5)
        ))
    }
}
